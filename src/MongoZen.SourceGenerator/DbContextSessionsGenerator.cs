...
public sealed class DbContextSessionsGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateGenericDispatch(StringBuilder sb, string indent, List<(string Name, string EntityType)> props, string methodName)
    {
        ...
        {
            var prop = props[i];
            sb.Append(indent).Append(i == 0 ? "if" : "else if").Append(" (entity is ").Append(prop.EntityType).Append(" e").Append(i).AppendLine(")");
            sb.Append(indent).AppendLine("{");
            sb.Append(indent).Append("    ").Append(prop.Name).Append(".").Append(methodName).Append("(e").Append(i).AppendLine(");");
            sb.Append(indent).AppendLine("}");
        }
        if (props.Count > 0)
        {
